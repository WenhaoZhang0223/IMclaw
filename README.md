...
本仓库以开源项目 [pi-mono](https://github.com/badlogic/pi-mono) 作为 Agent 基础设施。`pi-ai`、`pi-agent-core` 和 `pi-coding-agent` 来自上游项目，分别提供模型接入、Agent 工具循环和 Coding Agent 基础能力。

我独立设计并实现了 `packages/imclaw` 集成层，把 Pi 扩展成一个可以在飞书中长期运行的私人 AI Agent。我的主要工作包括：

- 飞书 WebSocket 长连接、消息解析、去重和主人身份验证
- 每个聊天独立的持久化 Agent Session 与 FIFO 任务队列
- Agent 生成文件的安全上传、飞书回传和失败保留机制
- `workfile` 私人文件库的自然语言检索、歧义确认和无损发送
- 任务超时、工具调用次数和后台浏览器启动次数限制
- 浏览器隔离规则、临时进程终止和临时文件清理
- PM2 后台部署、崩溃恢复和进程状态管理
- 面向关键路径、文件安全边界和飞书适配器的自动化测试
- IMclaw 架构、部署流程和安全设计文档

This repository uses the open-source [pi-mono](https://github.com/badlogic/pi-mono) project as its agent foundation. The upstream `pi-ai`, `pi-agent-core`, and `pi-coding-agent` packages provide model integration, the agent tool loop, and core coding-agent capabilities.

I independently designed and implemented the `packages/imclaw` integration layer, extending Pi into a private, long-running AI agent controlled through Feishu. My work includes:

- Feishu WebSocket connectivity, message parsing, deduplication, and owner authentication
- Persistent per-chat Agent Sessions with FIFO task queues
- Safe upload and delivery of generated files with failure-preservation semantics
- Natural-language search, ambiguity confirmation, and non-destructive delivery for the private `workfile` library
- Task timeouts, tool-call quotas, and headless-browser launch limits
- Browser isolation, temporary-process termination, and artifact cleanup
- PM2 deployment, crash recovery, and process management
- Automated tests for critical workflows, filesystem boundaries, and the Feishu adapter
- Architecture, deployment, and security documentation for IMclaw
